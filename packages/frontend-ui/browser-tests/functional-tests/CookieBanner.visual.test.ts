...
test.describe("Cookie Banner visual regression", () => {
  ...
  test("should match visual snapshot after accepting cookies", async ({
    page,
  }) => {
  ...
  test("should match visual snapshot after rejecting cookies and then hide cookie banner", async ({
    page,
  }) => {
    await page.reload();
