...
test.describe("Step Card visual regression", () => {
  ...
  test("should match visual snapshot - full page", async ({ page }) => {
    await expect(page).toHaveScreenshot("step-card-fullpage.png", {
      fullPage: true,
    });
  });
  ...
  test("should match visual snapshot at small screen size", async ({
    page,
  }) => {
  ...
  test("should match visual snapshot at very small screen size with stacked images", async ({
    page,
  }) => {
...
test.describe("Step Card functional", () => {
  ...
  test("should have aria-hidden on image containers", async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      await expect(imageContainers.nth(i)).toHaveAttribute(
        "aria-hidden",
        "true",
      );
    }
  ...
  test("images should stack vertically at very small screen size", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 280, height: 568 });
