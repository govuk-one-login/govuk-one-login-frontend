...
test.describe("Header component visual regression", () => {
  ...
  test('should take screenshots before and after clicking "Sign out"', async ({
    page,
  }) => {
    const signOutLink = page.locator(
      'a[href="https://signin.account.gov.uk/signed-out"]',
    );
    ...
    await expect(header).toHaveScreenshot(
      "header-after-signout-click-no-nav.png",
    );
    ...
    await expect(newHeader).toHaveScreenshot(
      "header-after-signout-navigated.png",
    );
  });
});
