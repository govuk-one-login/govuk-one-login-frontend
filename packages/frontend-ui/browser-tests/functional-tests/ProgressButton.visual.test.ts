...
test.describe("Progress Button visual regression", () => {
    ...
    await page.evaluate(() => {
      ...
      if (form) {
        form.addEventListener("submit", (event) => {
          console.log(
            "Playwright injected: Preventing default form submission!",
          );
  ...
  test("should match visual snapshot after clicking and waiting 5 seconds", async ({
    ...
    await page.waitForSelector(
      '.govuk-button--progress:has-text("Keep waiting")',
      { state: "visible" },
    );
    await expect(button).toHaveScreenshot(
      "progress-button-clicked-and-waited.png",
      { timeout: 10000 },
    );
  });

  test("should match visual snapshot in default state - reduced motion", async ({
    page,
  }) => {
    await page.emulateMedia({ reducedMotion: "reduce" });
    const button = await page.locator(".govuk-button--progress").first();
    await expect(button).toHaveScreenshot(
      "progress-button-default-no-motion.png",
    );
  });
  ...
  test("should match visual snapshot after clicking and waiting 5 seconds - reduced motion", async ({
    ...
    await expect(button).toHaveScreenshot(
      "progress-button-clicked-and-waited-no-motion.png",
      { timeout: 10000 },
    );
  });
});
...
test.describe("Progress Button inside govuk-button-group", () => {
  ...
  test("should match visual snapshot of button group in default state", async ({
    page,
  }) => {
    const group = page.locator(".govuk-button-group", {
      has: page.locator(".govuk-button--progress"),
    });
    await expect(group).toHaveScreenshot("progress-button-group-default.png");
  });

  test("should match visual snapshot of button group after clicking progress button", async ({
    page,
  }) => {
    const group = page.locator(".govuk-button-group", {
      has: page.locator(".govuk-button--progress"),
  ...
  test("sibling button does not move vertically when progress button is clicked", async ({
    page,
  }) => {
    const group = page.locator(".govuk-button-group", {
      has: page.locator(".govuk-button--progress"),
    ...
    await progressButton.click({ force: true });
    await progressButton.evaluate((el) =>
      el.classList.contains("govuk-button--progress-loading"),
    );
...
test.describe("Progress Button with JavaScript disabled", () => {
  ...
  test("should match visual snapshot of form button with JS disabled", async ({
    page,
  }) => {
    await page.goto("http://localhost:3000/test-progress-button");
  ...
  test("should match visual snapshot of button group with JS disabled", async ({
    page,
  }) => {
    await page.goto("http://localhost:3000/test-progress-button");
    const group = page.locator(".govuk-button-group", {
