...
test.describe("Progress Button visual regression", () => {
  ...
  test("should match visual snapshot after clicking and waiting 5 seconds", async ({
    ...
    const button = page.locator(".govuk-button--progress").first();
    await button.click({ force: true, button: "left" });
    await page.waitForSelector('.govuk-button--progress:has-text("Wait")', {
      state: "visible",
    });
  ...
  test.skip("should match visual snapshot after clicking and waiting 5 seconds - reduced motion", async ({
    page,
  }) => {
    await page.emulateMedia({ reducedMotion: "reduce" });
    const button = page.locator(".govuk-button--progress").first();
    await button.click({ position: { x: 10, y: 10 } }); // check it's not a position issue
    await page.waitForSelector('.govuk-button--progress:has-text("Wait")', {
      state: "visible",
    });
    await page.waitForTimeout(5100); // slightly over 5s to ensure the timeout has triggered
...
// looks like the link is active but not clicked
