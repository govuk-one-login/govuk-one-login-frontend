...
test.describe("Language Select component visual regression", () => {
  ...
  test("should match visual snapshot of language select component with English default", async ({
    page,
  }) => {
    const languageNav = page.locator("nav.language-select");
    await expect(languageNav).toBeVisible();
    await expect(languageNav).toHaveScreenshot(
      "language-select-default-en.png",
    );
  ...
  test("should match visual snapshot with Cymraeg selected", async ({
    page,
  }) => {
