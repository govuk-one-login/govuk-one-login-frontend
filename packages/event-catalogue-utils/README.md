...
@govuk-one-login/event-catalogue-utils is a set of functions to extract pass-through headers that should be passed through our frontends to our backends.
...
Add to your project using `npm i @govuk-one-login/event-catalogue-utils`
...
The event-catalogue-utils library allows you to set a custom logger or use the default pino logger.
...
import { setCustomLogger } from "@govuk-one-login/event-catalogue-utils";
...
If no custom logger is set, a pino logger is used with:
	•	Name: @govuk-one-login/event-catalogue-utils
...
Please raise any issues on the [GitHub repo](https://github.com/govuk-one-login/event-catalogue-utils).
